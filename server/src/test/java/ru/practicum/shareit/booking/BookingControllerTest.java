...
class BookingControllerTest {
    ...
    void should_get_by_booker_id() throws Exception {
        ...
        when(bookingService.getByBookerId(anyLong(), any(State.class), anyInt(), anyInt())).thenReturn(result);
        ...
        verify(bookingService, times(1)).getByBookerId(anyLong(), any(State.class), anyInt(), anyInt());
    }

    @Test
    void should_get_by_owner_id() throws Exception {
        List<BookingResponseDto> result = List.of(bookingResponseDto);
        when(bookingService.getByOwnerId(anyLong(), any(State.class), anyInt(), anyInt())).thenReturn(result);
        mvc.perform(get("/bookings/owner")
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasSize(1)));
        verify(bookingService, times(1)).getByOwnerId(anyLong(), any(State.class), anyInt(), anyInt());
    }
}